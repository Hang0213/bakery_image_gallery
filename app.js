...
const updatePage = async () => {
  ...
  const filtedFruitsArray = fruitsArray.filter((item) => 
    item.nutritions.sugar > 15
  );
  console.log(filtedFruitsArray)
  const fruitNameArrary = filtedFruitsArray.map(item => item.name);

  // TODO: Create a new HTML element to display your data 
  const fruitElement = document.getElementById('fruit');

  const fruitDisplayElement = document.createElement('div');
  fruitElement.append(fruitDisplayElement);
  ...
  fruitDisplayElement.appendChild(fruitList)
  console.log(fruitNameArrary)
  for(let i = 0; i < fruitNameArrary.length; i++){
    let li = document.createElement('li');
    li.innerHTML = fruitNameArrary[i];
    console.log(fruitNameArrary[i]);
